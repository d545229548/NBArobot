    ...
    public static String getOrderListText(List<String> orderItem) {
        if(orderItem.isEmpty()) {
            ...
            for(int i = 1; i <= orderItem.size() - 1; ++i) {
                sb.append(i).append(". ").append((String) orderItem.get(i - 1)).append("\n");
            }

            sb.append(orderItem.size()).append(". ").append((String) orderItem.get(orderItem.size() - 1));
            return sb.toString();
        }
    }
    ...
    public static String getUnorderListText(List<String> unorderItem) {
        if(unorderItem.isEmpty()) {
            ...
            for(int i = 0; i < unorderItem.size() - 1; ++i) {
                sb.append("- ").append((String) unorderItem.get(i)).append("\n");
            }

            sb.append("- ").append((String) unorderItem.get(unorderItem.size() - 1));
            return sb.toString();
        }
    }
